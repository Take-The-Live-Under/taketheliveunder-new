...
export default function GameCharts({
  ...
  // Try to fetch historical data from DB
  const fetchSnapshots = useCallback(async () => {
    try {
      ...
      if (result.timeline && result.timeline.length > 0) {
        ...
        // No DB data, use real-time collection
        setUseRealtime(true);
      }
    } catch {
      // DB not available, use real-time collection
      setUseRealtime(true);
    } finally {
      setLoading(false);
    }
  ...
  if (chartData.length < 2) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-green-800 text-[10px] mt-2">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-green-900/20 border border-green-900 p-4">
        <h3 className="text-xs font-semibold text-green-400 mb-4">
          // PACE_ANALYSIS
        </h3>
        <div className="h-64">
